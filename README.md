WEB APPLICATION RECON, ENUMERATION, FOOTPRINTING FRAMEWORK with the extended capability of both passive and active mode of attack on a targeted domain using- domain tools,analysing public archives and records,web crawler and information extractor,port scanning and waf identification ,wayback url etc..and so on developing capabilitys and features.

              !!!..MODEL IS UNDER DEVELOPMENT PHASE..!!!
